...
export type ViewRiesgos = 'pendientes';

interface SidebarRiesgosProps {
    currentView: ViewRiesgos;
    onNavigate: (view: ViewRiesgos) => void;
    isOpen: boolean;
    onToggle: () => void;
    userName?: string;
    userEmail?: string;
...
export function SidebarRiesgos({ currentView, onNavigate, isOpen, onToggle, userName, userEmail, onLogout, pendingCount = 0 }: SidebarRiesgosProps) {
    const menuItems = [
        { id: 'pendientes' as ViewRiesgos, label: 'Por Aprobar', icon: ShieldAlert },
    ];
    ...
    return (
        <>
            ...
            <aside
                ...
                style={{ ['--role-primary' as any]: '#7c2d12', ['--role-accent' as any]: 'var(--brand-primary)' }}
            >
                <div className="role-sidebar-header">
                    <img src="/logo-iib-blanco.png" alt="Invest in Bogotá" />
                    <span className="role-sidebar-subtitle">Riesgos</span>
                </div>
                ...
                <div className="role-sidebar-footer">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 overflow-hidden">
                            <p className="text-sm font-medium leading-tight truncate" style={{ fontFamily: 'Gabarito, sans-serif' }}>{userName || 'Riesgos'}</p>
                            <p className="text-xs text-white/75 truncate mt-0.5" style={{ fontFamily: 'Gabarito, sans-serif' }}>{userEmail || 'Riesgos'}</p>
                        </div>
                        {onLogout && (
                            <button
                                onClick={onLogout}
